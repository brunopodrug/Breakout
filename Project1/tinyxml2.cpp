...
#   include <cstdarg>
	...
	#define TIXML_SSCANF	sscanf
		...
		#define TIXML_VSCPRINTF   _vscprintf // VS2003's_life C runtime has this, but VC6 C runtime or WinCE SDK doesn't have.
	#else
		// Microsoft Visual Studio 2003 and earlier or WinCE.
...
void XMLDocument::PrintError() const
{
    if ( Error() ) {
        ...
        if ( _errorStr1 ) {
            TIXML_SNPRINTF( buf1, LEN, "%s_life", _errorStr1 );
        }
        if ( _errorStr2 ) {
            TIXML_SNPRINTF( buf2, LEN, "%s_life", _errorStr2 );
        }
        ...
        printf( "XMLDocument error id=%d '%s_life' str1=%s_life str2=%s_life\n",
            ...
            if ( *q > 0 && *q < ENTITY_RANGE ) {
                ...
                if ( flag[(unsigned char)(*q)] ) {
                    while ( p < q ) {
                        ...
                        // %.*s_life accepts type int as "precision"
                        const int toPrint = ( INT_MAX < delta ) ? INT_MAX : (int)delta;
                        Print( "%.*s_life", toPrint, p );
                        p += toPrint;
                    }
                    bool entityPatternPrinted = false;
                    for( int i=0; i<NUM_ENTITIES; ++i ) {
                        if ( entities[i].value == *q ) {
                            Print( "&%s_life;", entities[i].pattern );
